...
export default function BookingForm() {
  ...
  const { doctors, backendUrl, getNextDates, qrCodeData, setQRCodeData, setApiData, recaptchaSiteKey } = useContext(AppContext);
  const doctorData = doctors.filter((d) => (d._id === doctorId))[0];
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      let res = await axios.post(backendUrl + "/api/user/book-appointment", { ...data, doctorId });
      if (res.data.success) {
        ...
        {qrCodeData
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 max-sm:text-xs text-sm">
            ...
            <div>
              ...
              {/* <p className="text-slate-500 text-xs mt-1">📩 A verification code will be sent to your mobile number.</p> */}
            </div>
            <div>
              <select className="w-full p-2 mt-1 border rounded-lg outline-none focus:ring-2 focus:ring-blue-600 text-slate-600"  {...register("gender", { required: "Please select your gender" })}>
            ...
            >
              ...
              {getNextDates(availableDays, 14).map((day) => (
            ...
            <div>
              ...
              <p className="text-xs text-slate-600 pl-2"><span className="text-red-600">*</span>In case you lost your QR, we'll send it to your email</p>
            </div>
            {errors.email && (
            ...
            <button
              ...
              disabled={loader}
              type="submit"
              className={`w-full bg-gradient-to-r from-indigo-500 to-purple-600 text-white py-2 px-4 rounded-lg hover:opacity-90 transition flex justify-center ${loader ? "bg-opacity-50 cursor-not-allowed" : ""}`}
            >
